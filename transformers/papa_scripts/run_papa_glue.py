...
from transformers import papa_modules
...
class DataTrainingArguments:
    ...
    the command line.
    """
    use_papa_preprocess: bool = field(
        default=False, metadata={"help": "Use PAPA preprocess for seq classification datasets"}
    )
...
class ModelArguments:
    ...
    use_freeze_extract_pooler: bool = field(
        default=False,
        metadata={
            "help": "Whether to use the freeze pooler."
        },
    )

    grad_for_classifier_only: bool = field(
        default=False,
        metadata={
            "help": "Update the classifier only."
        },
    )

    grad_for_classifier_and_static_only: bool = field(
        default=False,
        metadata={
            "help": "Update the classifier and static matrices only."
        },
    )

    static_heads_dir: str = field(
        default=None,
        metadata={
            "help": "The static heads dir."
        },
    )

    sorting_heads_dir: str = field(
        default=None,
        metadata={
            "help": "The heads sorting dir."
        },
    )

    static_heads_num: int = field(
        default=0,
        metadata={
            "help": "The number of static heads."
        },
    )

    sort_calculating: bool = field(
        default=False,
        metadata={
            "help": "The number of static heads."
        },
...
def main():
    ...
    if model_args.sort_calculating:
        config.sort_calculating = {'heads_dir': model_args.static_heads_dir}
    ...
    if config.static_heads:
        model.prune_static_heads()
    ...
    if model_args.grad_for_classifier_only:
        for param_name, param in list(model.named_parameters()):
            if param_name.startswith("classifier") or "pooler" in param_name:
                print("Not frozen param: ", param_name)
                continue
            param.requires_grad = False

    if model_args.grad_for_classifier_and_static_only:
        for param_name, param in list(model.named_parameters()):
            if param_name.startswith("classifier") or "pooler" in param_name or "static_heads_content" in param_name:
                print("Not frozen param: ", param_name)
                continue
            param.requires_grad = False
    ...
    def add_preprocess_func(result):
        ...
        result.data['old_input_ids'] = result.data['input_ids']
        result.data['old_attention_mask'] = result.data['attention_mask']
        ...
        for i, cur_input_ids, cur_att_mask in zip(range(len(input_ids)), input_ids, att_mask):
            ...
            for j in range(first_sep_index, last_sep_index):
                if max_seq_length // 2 + j - first_sep_index == len(new_input_ids[i]) - 1:
                    break
                new_input_ids[i][max_seq_length // 2 + j - first_sep_index] = cur_input_ids[j]
                new_att_mask[i][max_seq_length // 2 + j - first_sep_index] = 1

            new_input_ids[i][-1] = cur_input_ids[last_sep_index]
            new_att_mask[i][-1] = 1


        result.data['input_ids'] = new_input_ids
        result.data['attention_mask'] = new_att_mask
    ...
    def preprocess_function(examples):
        ...
        if data_args.use_papa_preprocess:
    ...
    if training_args.do_train:
        ...
        if model_args.sort_calculating:
            lambdas = np.array([torch.squeeze(l.attention.self.lambdas).cpu().detach().numpy() for l in model.base_model.encoder.layer])
            np.save(os.path.join(training_args.output_dir, "lambdas"), lambdas)
            lambdas_sorted = np.argsort(lambdas.flatten())
            with open(os.path.join(training_args.output_dir, "sorted_heads.json"), "w") as fp:
                heads_sorted = [[int(h // config.num_attention_heads), int(h % config.num_attention_heads)] for h in lambdas_sorted]
                json.dump(heads_sorted, fp)
    ...
    if training_args.do_eval:
        ...
        for eval_dataset, task in zip(eval_datasets, tasks):
            ...
            if task.startswith('mnli'):
                split = eval_dataset.split._name
    ...
    if training_args.do_predict:
        ...
        for predict_dataset, task in zip(predict_datasets, tasks):
            ...
            split = "test"
            if task.startswith('mnli'):
                split = predict_dataset.split._name
            trainer.log_metrics(split, metrics)
            trainer.save_metrics(split, metrics)
