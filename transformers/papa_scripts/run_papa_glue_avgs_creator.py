...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Use papa preprocess for glue dataset.",
    )
...
def main():
    ...
    with torch.no_grad():
        attentions_sums = torch.zeros((config.num_hidden_layers, config.num_attention_heads, args.max_length, args.max_length)).to(device)
        attention_patterns_sums = torch.zeros((args.max_length, args.max_length)).to(device)
        model.eval()
        ...
        for epoch in range(1):
            for step, batch in enumerate(train_dataloader):
                ...
                loss = outputs.loss
                att_mask = batch['attention_mask'].float()
                cur_attention_pattern = torch.bmm(torch.unsqueeze(att_mask, -1), torch.unsqueeze(att_mask, 1))
                attention_patterns_sums += torch.sum(cur_attention_pattern, dim=0)

                cur = torch.zeros((cur_attention_pattern.shape[0], config.num_hidden_layers, config.num_attention_heads, args.max_length, args.max_length)).to(device)

                for l in range(config.num_hidden_layers):
                    cur[:, l] = outputs.attentions[l]
                cur = cur * torch.unsqueeze(torch.unsqueeze(cur_attention_pattern, 1), 1)
                attentions_sums += torch.sum(cur, dim=0)

        final_avgs = torch.nan_to_num(attentions_sums / attention_patterns_sums).to(device)
        #save!!
        torch.save(attentions_sums.cpu(), os.path.join(args.output_dir, "attention_sums.pt"))
        torch.save(attention_patterns_sums.cpu(), os.path.join(args.output_dir, "attention_patterns_sums.pt"))
        torch.save(final_avgs.cpu(), os.path.join(args.output_dir, "avgs.pt"))
